# [Forward algorithm](https://www.taylorfrancis.com/books/mono/10.1201/b20790/hidden-markov-models-time-series-walter-zucchini-iain-macdonald-roland-langrock) for hidden semi-Markov models with periodically varying transition probability matrices
...
and with structured transition probabilities. Recently, this inference
procedure has been generalised to allow either the dwell-time
distributions or the conditional transition probabilities to depend on
external covariates such as the time of day. This special case is
implemented here. This function allows for that, by expecting a
transition probability matrix for each time point in a period, and an
integer valued (\\1, \dots, L\\) time variable that maps the data index
to the according time.
