...
We now model the transition probabilities parametrically, where we have a parameter for the intercept, the linear effect and the quadratic effect for each off-diagonal element of the t.p.m.
...
Again, we use `tpm_g()` and `stationary()` to transform the parameters.
...
<!-- ### Non-parametric modelling of the transition probabilities -->
...
<!-- Again, we use `tpm_g()` and `stationary()` to transform the unconstrained parameters to working parameters. -->
...
> Continue reading with [**Periodic HMMs**](https://janolefi.github.io/LaMa/articles/Periodic_HMMs.html), [**LaMa and RTMB**](https://janolefi.github.io/LaMa/articles/LaMa_and_RTMB.html), or [**Penalised splines**](https://janolefi.github.io/LaMa/articles/Penalised_splines.html).

<!-- ### Non-parametric modeling of the state-dependent regressions -->

<!-- This is now a trivial task, just combining the previous two examples. -->
